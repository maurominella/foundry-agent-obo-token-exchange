...
- **Token Exchange URL** = the App ID URI of the **same** registered application (`api://botid-2486b5cf-28b0-4f2d-b7c8-ff71aa856b72`). This is the `aud` claim of the **incoming** SSO token generated toward the Azure Bot by Entra following Teams' request. It tells Entra: *"you must exchange (OBO) the token received for the Bot's registered application, building another one with this `aud`."* OBO works only if the app performing the exchange is the **same one that owns the `aud`** of the incoming token. Since the Token Exchange URL contains `botid-{Bot appId}`, the only app that owns that audience is the Bot's registered application. Therefore `client_id` + secret must be the Bot's. **You cannot separate them.**
- `client_id`, `client_secret`, and `tenant_id` of the registered application associated with our custom bot web service, so it can authenticate as our registered application, which is a **confidential** application.
- The **Scope** the bot must ask Entra to mint in the outgoing token, i.e. `https://ai.azure.com/.default` — Foundry's scope. This way Entra places `aud = https://ai.azure.com` and `scp = user_impersonation` in the **outgoing** token toward Foundry.

This connection lets the Bot use the **Bot Framework Token Service** to perform a genuine OBO — swapping the Teams/Copilot user's SSO token for a token of the same user valid for Foundry (`aud = ai.azure.com`). The Bot Framework Token Service uses `client_id` and `client_secret` to authenticate as the bot's confidential application and ask Entra for the OBO exchange: *swap this Teams token for a Foundry token.*

![Azure Bot — OAuth Connection (AAD v2) for Foundry](images/04-05-azure-bot-oauth-connection-foundry.png)
*Azure Bot — OAuth Connection (AAD v2) for Foundry*
